...
| 5 | dbt-core 도입, Silver→Gold 일부 dbt 이관, dbt tests 5~10개, GitHub Actions CI | CI green + DQ 리포트 |
...
| 9 | **Spark batch** Iceberg MERGE INTO 멱등성 검증 + Compaction(`rewrite_data_files`) + 비용 측정 | 1번 미해결 이슈 closure 증거 |
| 10 | 아키텍처 다이어그램 + README + **포트폴리오 1차 작성 (Phase 1A 단독, 5~6p)** | **체크포인트 1: 포트폴리오 제출 가능** |
...
### 7-5. 1A+1B 통합 포트폴리오 (8~10p)

위 Phase 1A 6p + 다음 추가:
7. 익명 실서비스 아키텍처 (Edge API + REST Proxy + D1 + Web Push)
8. `user.events.v1` 처리 + 익명 ID 거버넌스
9. Phase 1B 트러블슈팅 (Web Push / D1 / Edge API → Kafka 등)
10. 본인 운영 실서비스 + 그 서비스의 데이터 플랫폼 통합 어필 + Phase 2 로드맵
...
| 면접관 카운터 | 응답 |
|---|---|
| "1번이랑 뭐가 다른가요?" | streaming 엔진(Flink), CDC(Debezium), 변환 레이어(dbt), CI(GitHub Actions), 카탈로그(Lakekeeper), 쿼리 엔진(DuckDB) — **6개 영역이 다름**. |
...
| 9 | Spark 셋업 안 됨 | PyIceberg 또는 dbt MERGE 매크로로 멱등성 검증. |
...
- **메모리 충돌**: Kafka KRaft + Flink 상시, Spark는 Day 9 일시 기동. Day 1에 free 메모리 측정 스크립트 작성. 80% 초과 시 알림.
