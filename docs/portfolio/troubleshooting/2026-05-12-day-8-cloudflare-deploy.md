# Day 8 운영 후속 — Cloudflare Pages 실 배포 + 임시 Tunnel (PR γ-2)

> 작성: 2026-05-12 01:00 KST
> 시점: Day 8 PR α (#45) + hotfix (#46) + PR β (#47) + hygiene (#48, #49) 모두 머지 후, Cloudflare Pages 실 배포 + Cloudflare Tunnel의 사용자 수동 (Day 7 PR γ §10-5 SoT). 본 archive 는 Cloudflare의 troubleshooting 자산 명문화.
> 관련 PR: 본 PR γ
> 직전 archive: [`2026-05-12-day-8-archive.md`](2026-05-12-day-8-archive.md) (Day 8 코드 학습)
> 사용자 수동 작업 SoT: Day 7 PR γ archive [`2026-05-11-day-7-nextjs-cloudflare-deploy.md`](2026-05-11-day-7-nextjs-cloudflare-deploy.md) §10-5

## 0. 진입 흐름 요약

Day 8 코드 종료 (PR #45-49 머지) 후 Cloudflare Pages 실 배포 진입. Day 7 PR β 시점에는 docs / config만 정착, 실 배포 명령 (`wrangler pages deploy`) 은 사용자 수동으로 분리됐던 항목. 본 시점에 사용자가 Cloudflare 계정 가입 + 실 배포 진행.
...
## 1. Issue 1 — Workers vs Pages 진입 정공

### 증상

Cloudflare 가입 후 첫 진입 시 사용자가 "Create a Worker" 흐름 선택. GitHub repo (`benidjor/seoul-citydata-platform`) 연결 + project name (`seoul-citydata-platform`) 입력 + Deploy command (`npx wrangler deploy`) 에서 **HTTP 403** 차단.

![Workers 흐름의 HTTP 403](./2026-05-12-day-8-cloudflare-deploy/screenshots/01-workers-403.png)

### 진단

Cloudflare 의 두 product:

| 항목 | Workers | Pages |
|---|---|---|
| **용도** | 동적 JS edge runtime (serverless function, API) | 정적 site 호스팅 (HTML/JS/CSS) |
| **Phase 1A 적용 대상** | (Phase 1B 의 Edge API `user.events.v1`) | **frontend `web/out/` 정적 산출** ← 본 작업 |
| **deploy 명령** | `npx wrangler deploy` (wrangler.toml 의존) | `wrangler pages deploy <dir>` 또는 GitHub 자동 연동 |

본 repo 에는 `wrangler.toml` 부재 (Phase 1B 진입 전, CLAUDE.md §3 SoT) → Workers 흐름의 `npx wrangler deploy` 가 config 못 찾음 → 403 차단 추정.
...
### 학습 — Cloudflare 의 두 product 구분 의무

Cloudflare 의 최근 UI 변경 — Workers 와 Pages 가 통합된 흐름 (Workers & Pages) 으로 변경되면서 처음 "Create" 진입 시 default 가 Worker. 정적 site은 Pages 명시 의무. 사용자 첫 진입 시 흔한 trap.
...
502 transient 발생 + retry 에서 결과적으로 "Success" 보고. 그러나 실 URL (`seoul-citydata-platform.pages.dev`) 접근 시 **500 Internal Server Error**.

![1차 deployment details — Success 보고](./2026-05-12-day-8-cloudflare-deploy/screenshots/03-first-deploy-details.png)

페이지 자체는 텍스트만 보이고 Leaflet 지도 / 리스트 부분 비어있음:
...
wrangler 가 deploy step 의 31 file upload 중 502 받음 → retry → 결과적으로 "Success" 보고했지만 실제로는 일부 chunk file 이 **corrupt으로 upload**. Cloudflare Pages CDN 에서는 누락 또는 0 byte file → 500 반환.

### 학습 — "빌드 성공" ≠ "산출 정상"

빌드 status ✓ 는 git 연동 + 빌드 명령 + wrangler 호출 완료. 실 파일 무결성은 별개. 본 502 transient 같은 인프라 이슈는 wrangler 의 retry이 cover 못 함.

**진단 의무** — 빌드 success 후 다음 항목 cross-check:
1. **빌드 로그의 `Uploaded X files (Y already uploaded)`** — 이 값이 정공 fresh upload 검증의 SoT
2. **실 URL 접근 + 네트워크 탭의 chunk status** — HTML 200 + chunk 500 = upload corrupt 신호
3. **브라우저 콘솔의 `<script> 로드에 실패`** — chunk 500 의 client side 영향
...
2차 빌드 + deploy 진행 → status Success → 그러나 **chunk 여전히 500**. 같은 corrupt chunk 그대로 노출.
...
**31 file 중 23 file 이 `already uploaded`** = Cloudflare Pages 가 chunk file hash 기준 dedup → 이전 deploy 의 corrupt cache hit → 같은 corrupt chunk 그대로 노출.

### root cause — Next.js 의 deterministic build + Cloudflare 의 hash dedup

Next.js 14 의 content-based chunk hash — **코드 변경 0 = 같은 hash = 같은 file name**. git 빈 commit 은 코드 변경 X → chunk hash 동일 → Cloudflare 가 "이미 같은 hash 의 file 있음, skip" 처리.

따라서:
- 1차 deploy 의 502 corrupt chunk → cache에 보존
- 2차 / 3차 deploy 의 dedup → cache의 corrupt 재사용
- **빈 commit 으로는 영원히 fix 안 됨**

### 학습 — Cloudflare Pages file dedup cache의 함정

본의 trap 인지가 정공:
- "Retry deployment" 또는 "git 빈 commit" 으로는 **content 변경 없는 chunk fix 불가**
- chunk hash 강제 변경 의무 — 코드 변경 또는 buildId 변경 또는 cache 자체 reset

## 4. Issue 4 — asdf 의 NODE_VERSION patch resolve
...
Cloudflare Pages build environment 의 **asdf version manager**:
...
- 16.20.2 (구 LTS, EOL)
- 18.17.1 (구 LTS)
- **20.19.0** (현재 LTS, next 14 권장) ← 본 작업
- 22.16.0 (latest LTS)

### 학습 — Cloud build environment 의 default version manager

Cloudflare Pages / Netlify / Vercel 같은 cloud build의 default behavior:
- **default Node version** = 보통 LTS 의 어떤 버전 (cloud 마다 다름). 본 Cloudflare = 18.x default
- **major.minor만 입력 시 latest patch resolve**: asdf 의 동작 — 1차 시도에서 `20` → `20.20.0` 으로 resolve. 본 환경 preset에 없으면 fail
- **권고** = 정확한 `<major>.<minor>.<patch>` 명시 (예: `20.19.0`). cloud의 preset 변경에 영향받지 않음

Day 7 PR γ `2026-05-11-day-7-nextjs-cloudflare-deploy.md` §1 의 "pnpm 11 의 ignored build script 정책 변경" 과 같은 패턴 — **cloud / package manager 의 default 변경이 plan 본문 가정과 충돌**.

## 5. Issue 5 — NEXT_PUBLIC_API_BASE build-time inline + 부분 fresh upload

### 증상

Issue 4 fix (NODE_VERSION=20.19.0) 후 빌드 성공. 그러나 chunk 여전히 500. 3차 fix 시도 — **NEXT_PUBLIC_API_BASE=`https://api.placeholder.invalid` 환경변수 추가** + Retry deployment.
...
11 file fresh upload — 이전 8 file 보다 증가 ✓. 일부 chunk hash 변경 효과 확인.
...
### 진단 — Next.js 의 chunk 종류

Next.js 14 의 빌드 chunk 분류:

| chunk | hash 변경 trigger |
|---|---|
| **webpack runtime** | next.config 변경 또는 buildId 변경 |
| **vendor (node_modules)** | 의존성 추가/제거/버전 변경 |
| **main-app** | `app/layout.tsx` 등 app shell 변경 |
| **page chunk** | 해당 page tsx 변경 |
| **CSS** | Tailwind output (style 사용 변경) |

→ 환경변수 inline (webpack runtime + 일부 page) 만 변경 영향. vendor chunk 는 의존성 변경 없으면 영원히 같은 hash.

### 결론 — chunk hash 부분 변경으로는 영원히 fix 불가

Cloudflare Pages 의 dedup이 hash 기준이라 vendor chunk 같이 변경 없는 부분은 영원히 cache hit. 다음 옵션 의무:

1. **Pages project 재생성** (cache 완전 reset) ← 본 archive 의 정공
2. **wrangler API 로 deployment file purge** (가능 검토)
3. **vendor 의존성 강제 변경** (over-engineering)
4. **`generateBuildId` 명시 + 모든 chunk 영향** (Next 14 의 buildId 는 chunk hash 직접 영향 X)

## 6. Issue 6 — Pages project 재생성 (nuclear option) 정공

### 진행

기존 Pages project 의 cache이 영원히 corrupt vendor chunk 보존 → 유일한 해결 = **project 자체 재생성**:
...
**31 file 모두 fresh upload, 0 already uploaded** ✓ — 새 project 의 cache 는 비어있으므로 모든 chunk fresh upload.
...
본 작업은 다음 에서 의무:
- **502 transient 같은 인프라 이슈 영향으로 chunk corrupt 가 hash에 보존된 경우**
- chunk hash 부분 변경의 fix 시도 후에도 잔존 chunk corrupt
- wrangler / 대시보드의 cache purge이 부재한 경우

**대안 inventory** (본 archive 에서 시도 안 함, 추후 검토):
- `wrangler pages deployment delete <id>` — 개별 deployment 삭제
- `wrangler pages deployment list` + 모든 deployment 삭제 후 fresh push
- Cloudflare 대시보드의 cache invalidation (Settings → Build cache)

## 7. Issue 7 — 임시 Cloudflare Tunnel 가동 + 실 작동 검증

### 진행

Pages project 재생성 정공 + frontend 정적 노출 결과 정공 → 다음 항목 = **API tunnel** (Day 7 PR γ §10-5 의 사용자 수동).

도메인 결정 — 사용자 도메인 부재 → 4 옵션 검토:
...
| 저비용 도메인 | `*.dev` 등 | $10/년 | ✅ |
| Workers | `*.workers.dev` | $0 | ✅ (Phase 1B) |

본 시점 = Phase 1A 데모 → **임시 tunnel 선택** (가장 빠른 setup, Phase 1B / 2 에서 영구 방식 검토 분리).
...
tunnel connection 등록 — 인천 (ICN05), 빠른 latency.
...
| **Plus** — Cloudflare 실 배포 + Tunnel 실 작동 | ✅ Phase 1A 데모 완전 정공 |

## 8. 운영 인지 의무

임시 tunnel의 영구 작동 위해 다음 4 process alive 유지 의무:
...
- **macOS sleep 진입** — caffeinate 가 cover. 만료 시 sleep 진입 → cloudflared / uvicorn 도 sleep
- **cloudflared 재가동 시 새 hostname** — 매 재가동 시 `*.trycloudflare.com` 새 URL → Cloudflare Pages 환경변수 + Retry deployment 의무 (자동화 불가)
- **사용자 인터넷에 의존** — 사용자 macOS 의 인터넷 연결이 API 노출. WiFi disconnect / ISP issue 시 API 접근 불가
- **uptime 보장 없음** — Cloudflare 의 `trycloudflare.com` = production 용 아님 (cloudflared 로그 line 1 의 명시)

### Phase 1B / 2의 영구 방식 진행 권고

본 임시 tunnel의 제약 = Phase 1A 데모 한정 (포트폴리오 스크린샷 / 시연 영상 cover). 본격 외부 공개 진입 시 다음 항목 의무:

1. **DuckDNS 또는 저비용 도메인 ($10/년)** — 영구 hostname
2. **정식 named tunnel** (`cloudflared tunnel create` + `cloudflared tunnel route dns`) — 영구 방식 + DNS 매핑
3. **launchd 의 자동 가동** — 사용자 macOS 재부팅 시 cloudflared 자동 재가동
4. **모니터링** — Tunnel health check + 알림 (Phase 2)

## 9. 학습 패턴 5종

### 9-1. Cloudflare 의 Workers ≠ Pages 구분 의무

본 archive Issue 1 SoT — 정적 site = Pages, 동적 edge function = Workers. 첫 진입 시 default 가 Workers이라 사용자 혼란 가능. 명확한 product 인지가 trap 회피 정공.

### 9-2. 빌드 success ≠ 실 산출 정상

Issue 2 SoT — wrangler 의 retry이 인프라 transient (502) 의 corrupt 영향 cover 못 함. 실 산출 검증 SoT:

1. 빌드 로그의 `Uploaded X files (Y already uploaded)`
2. 실 URL 접근 + 네트워크 탭의 chunk status
3. 브라우저 콘솔의 `<script> 로드에 실패`

### 9-3. Cloudflare Pages file dedup cache의 영구 영향

Issue 3 SoT — chunk hash의 cache hit 이 502 transient corrupt을 영구 보존. 빈 commit / Retry deployment / 부분 hash 변경 등으로는 fix 불가. **Pages project 재생성**이 nuclear option 정공.

대안 (추후 검토):
- `wrangler pages deployment delete <id>` 로 개별 deployment cache reset
- 대시보드의 cache invalidation (Settings 검색 필요)

### 9-4. asdf / 패키지 매니저 의 default version resolve 함정

Issue 4 SoT — cloud build environment 의 `NODE_VERSION=20` 같은 major/minor 입력의 자동 patch resolve이 본 환경의 preset과 불일치 가능. 정확한 `<major>.<minor>.<patch>` 명시이 안전.

본 작업은 Day 7 PR γ archive §1 의 "pnpm 11 의 ignored build script 정책 변경" 과 같은 fingerprint — **cloud / package manager 의 default 변경이 plan 본문 가정과 충돌**.

### 9-5. NEXT_PUBLIC_API_BASE build-time inline의 chunk hash 영향 분리

Issue 5 SoT — Next.js 14 의 chunk 종류 (webpack runtime / vendor / main-app / page / CSS) 별로 hash 영향 trigger 다름. NEXT_PUBLIC_API_BASE 같은 build-time inline 변경 시:

- ✅ inline chunk (webpack + 일부 page) → hash 변경
- ❌ vendor / main-app / CSS chunk → 변경 없음, hash 동일

따라서 cache dedup 우회 위해서는 영향 분리 인지 + 모든 chunk 영향 의무. nuclear option (project 재생성) 정공.

본 작업은 Day 7 PR γ §4 의 "NEXT_PUBLIC_API_BASE build-time inline 위험" SoT 의 확장형 — 그 시점에는 fallback `localhost:8000` 의 prod chunk inline 위험 명문화, 본 시점에는 chunk hash 영향 분리 추가 학습.

## 10. 관련 문서

- 본 archive 의 직전: [`2026-05-12-day-8-archive.md`](2026-05-12-day-8-archive.md) (Day 8 코드 학습)
- Day 7 PR β archive (Cloudflare 자동화 docs 정착 시점): [`2026-05-11-day-7-nextjs-cloudflare-deploy.md`](2026-05-11-day-7-nextjs-cloudflare-deploy.md)
  - §4 — NEXT_PUBLIC_API_BASE build-time inline 위험 SoT
  - §10-5 — 자동화 vs 사용자 수동 분리
...
| 02 | `screenshots/02-pages-settings.png` | Pages Settings 진입 |
