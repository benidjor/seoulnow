...
> 영역: Day 8 PR α (#45 chill-open mart) + hotfix (#46 streaming long-running) + PR β (#47 /chill 페이지) + hygiene 2건 (#48 backend + #49 frontend) 머지 후의 학습 자산 명문화.
> 관련 PR: 본 PR γ (docs only)
> 후속 archive: [`2026-05-12-day-8-cloudflare-deploy.md`](2026-05-12-day-8-cloudflare-deploy.md) (Cloudflare Pages + Tunnel 영역, 스크린샷 12장 포함)
...
implementer 가 sanity 단계에서 두 falsify 영역 발견:

1. **`iceberg_scan('s3://...')` 의 Lakekeeper UUID-prefix path 영역 미해결** — Day 4 archive `2026-05-09-day-4-tasks-4_1-4_3.md` §plan deviation 4 SoT
2. **`dbt.ref('dim_place')` 의 Relation 객체 f-string interpolation 시 데이터 dump 영역** — dbt-duckdb python model 의 내부 동작, 본 시점에 첫 발견

### 1.2 사후 우회 영역

#### Deviation 8.2-A (사전 채택) — `iceberg_scan` → `table_paths` 우회

Day 4/5/6/7 archive SoT reuse — Lakekeeper REST 가 vend 하는 UUID-prefix path 영역을 `flink_jobs.lib.duckdb_iceberg.table_paths()` 가 catalog 의 metadata 에서 resolve. dispatch prompt 에 사전 채택 명시.

#### Deviation 8.2-B (사후 우회) — dbt SQL model → Python model + `to_view` 패턴

implementer 가 dbt SQL model 의 `{{ ref('dim_place') }}` jinja 영역 + `iceberg_scan` 직접 호출 영역 모두 falsify 후 Python model 로 변환:
...
def model(dbt, session):
    ...
    # 1) dbt.ref('dim_place') → Relation 객체. f-string 영역에 직접 박으면 데이터 dump.
    #    to_view 영역으로 view 등록 후 SQL 안에서 view name 으로 참조.
    dim_place_relation = dbt.ref("dim_place")
    dim_place_relation.to_view("dim_place_view", replace=True)

    # 2) Lakekeeper UUID-prefix path 영역은 lib `table_paths()` 우회
...
### 1.3 학습 가치 — 기존 dbt python model 영역 비교

본 시점까지 dbt python model 영역의 consumer:
...
| **`chill_open_now.py` (Day 8)** | PR #45 | gold mart, **`dbt.ref('dim_place')` cross-ref 영역 첫 사례** |

→ 기존 영역은 다른 dbt model 의 ref 없음 → `to_view` 패턴 부재. 본 시점에 첫 cross-ref 영역 → **신규 학습 자산** 정착.

### 1.4 Phase 1B / 2 영역 reuse

Phase 1B (`user.events.v1` 의 anonymous 사용자 행동 mart) / Phase 2 (UGC 별점 + Google Places merge mart) 영역의 dbt mart 가 다른 dbt python model 의 ref 영역 사용 시 본 패턴 reuse 의무:
...
PR #28 (Day 5 진입 전 lib 추출) 시점부터 정착된 lib `flink_jobs.lib.duckdb_iceberg.{build_catalog, configure_duckdb, table_paths}` 의 consumer 영역:
...
→ 본 PR α 영역에서 **3 consumer 동시 정착**. lib 의 SQL injection 표면 일원화 효과 확장.

### 2.2 학습 — lib reuse 영역의 가속

Day 7 PR γ §10-4 SoT — "lib 추출 후 매 PR 마다 consumer 추가" 패턴 정착. 본 PR α 시점의 8번째 consumer 정착 = drift 위험 0 + 의존성 명확.

Phase 1B / 2 영역의 신규 streaming / API / mart 영역 추가 시 같은 lib 위임 의무.
...
| smoke 검증 영역 | default 의존 | **명시적 `FLINK_SMOKE_RUN_SECONDS=600` export 의무** |
| 기존 `=90000` 우회 영역 | 25h smoke | (동일, smoke mode 진입) |

호환성 영역 — Day 1~7 의 smoke 검증 절차 (default 600 의존) 호환성 깨짐 → hygiene PR #48 의 runbook 영역 갱신 (§5 참조).

### 3.4 새 streaming job 추가 시 reuse 의무

Phase 1B (`user_events` consumer) / Phase 2 (버스 위치 / UGC streaming) 영역의 신규 streaming job 추가 시 본 `wait_for_shutdown()` 위임 의무. silent exit fingerprint 영역 회피 정공.

### 3.5 학습 — design boundary 의 명시적 closure 영역

PR δ §6-3 SoT — 코드 주석 / docstring 의 "검토" / "임시" / "smoke" / "TODO" 표현은 future 자산 영역. 본 hotfix 가 그 boundary 의 closure 정공 — bronze_to_silver.py:196 의 "운영 시점에는 별도 deploy mode (per-job cluster) 로 변경 검토" 영역 폐기 + lib 위임 정착.
...
| # | 영역 | Plan 원안 | 적용 | 사유 |
|---|---|---|---|---|
| 1 | `biz_reg_no` 타입 | `string` | **`number`** | PR #45 의 `_row_to_dict` 영역이 datetime 만 isoformat 변환, 나머지 raw 타입 보존. 실 응답 = Integer (`2208612001`) |
...
본 영역 = **사후 우회** (Day 5/6/7 archive 의 명시 SoT 없음, implementer 의 schema cross-check 가 첫 falsify 시점).

### 4.4 학습 — frontend 영역의 API response cross-check 의무

Day 7 PR α 영역에서는 plan 본문의 응답 cols (`district, gu_code, window_start, area_count, ...`) 가 실 sink DDL 과 1:1 일치 → 사후 우회 미발생 (Day 7 PR γ §2 의 deviation E 미발생 SoT). 본 PR β 영역에서는 plan type 정의가 PR #45 의 `_row_to_dict` 동작과 불일치 → 사후 우회 발생.

**frontend Item type 영역의 정공 검증** — implementer dispatch prompt 에서 `curl <api> | python3 -m json.tool` 의 실 응답 schema 와 1:1 cross-check 의무 명시 (Day 7 PR γ §10-1 의 통합 검증 SoT 의 frontend 확장형).
...
본 영역은 PR #45 의 scope discipline 으로 미처리 (PR γ §10-5 SoT) → 별도 hygiene PR 영역 분리. 본 hygiene PR 에서 `ruff format src/api/deps.py src/api/routes/hotspots.py` 적용 (`main.py` 는 PR #45 의 style fix commit `3298fcd` 에 포함된 영역).
...
기존 Day 1~7 의 smoke 검증 절차 (default 600 의존) 호환성 깨짐 → runbook 영역 명문화 의무.

### 5.2 hygiene frontend (#49)

`web/.eslintrc.json` 부재 → `pnpm lint` 가 interactive `next lint --init` prompt 진입 → 자동 검증 영역 차단. 본 hygiene 으로 `next/core-web-vitals` extends 1 file 정착:
...
- entity escape (`&middot;`, `&larr;` 영역) + key prop + a11y 영역의 자동 회귀 0 보장

### 5.3 학습 — scope discipline 분리 정공 (3 PR 영역)

본 hygiene 분할 영역의 의의:
- **(a) main HEAD baseline 회귀** = PR #45 scope 외 → 별도 PR 분리
- **(b) smoke runbook 갱신** = hotfix #46 의 후속 영역, 본 영역의 의존성 = PR #46 의 default 변경
- **(c) web/.eslintrc 정착** = PR #47 (frontend) 의 후속 영역, scope 외

3 영역의 자연 경계:
- (a) + (b) = backend 영역 → 1 PR (#48)
- (c) = frontend 영역 → 1 PR (#49)

scope discipline 의 정공 패턴 — 한 PR = 한 논리 작업 + 자연 경계 영역 분리. Day 7 PR β archive §10-5 SoT (자동화 vs 사용자 수동 영역 분리) 의 frontend / backend 확장형.
...
본 archive 영역의 SLO 영역은 19:45 fire 후 별도 commit 으로 보강 예정.

### 6.3 fire 후 진행

```bash
cat $(ls -t /tmp/slo-24h-*.log | head -1)
# 출력 영역 본 archive §6.2 로 반영 + 별도 commit
```

P95 < 420 통과/위반 결과에 따라:
- **통과** = Phase 1A 의 SLO 영역 일관 달성 → 포트폴리오 SLO 페이지 영역 강화
- **위반** = 원인 진단 (fixture 시차 / streaming 가동 영역의 latency / 측정 시점의 burst 영역 등) + Phase 1B 진입 전 fix 의무 영역

## 7. 학습 패턴 5종

### 7-1. dbt-duckdb python model 의 cross-ref + `to_view` 패턴 (신규 자산)

본 archive §1 SoT — `dbt.ref('other_model').to_view('view_name', replace=True)` 패턴이 다른 dbt python model 의 ref 영역 사용 시 필수. Relation 객체의 f-string interpolation 영역에서 발생하는 데이터 dump 영역 회피.

Phase 1B / 2 영역의 dbt mart 가 다른 dbt python model 의 ref 영역 사용 시 본 패턴 reuse.

### 7-2. `lib/lifecycle.py` 추출 = streaming long-running 영역의 단일 SoT

본 archive §3 SoT — 3 streaming job 의 동일 `time.sleep(SMOKE_RUN_SECONDS)` 패턴 → `wait_for_shutdown()` 위임. 다음 영역의 정공:

- silent exit fingerprint 영역 회피 (1h heartbeat + SIGTERM graceful)
- `FLINK_SMOKE_RUN_SECONDS=0` default 의 long-running mode + `>0` 영역의 smoke mode 양립
- Phase 1B 신규 streaming job 추가 시 reuse 의무

본 lib reuse 패턴 — `lib/duckdb_iceberg` (PR #28) 의 정공 reuse 패턴 SoT 와 정합. 별도 lib 영역 (`lib/lifecycle`) 정착의 첫 사례.

### 7-3. Plan 본문 vs 실 응답 schema cross-check 의 frontend 확장

본 archive §4 SoT — Day 7 PR γ §10-1 의 "통합 검증 single source" 패턴이 frontend Item type 영역에 확장:

1. implementer dispatch 시 `curl <api> | python3 -m json.tool` 의 실 응답 schema cross-check 의무
2. Plan 본문의 type 정의 vs 실 응답 keys 영역 1:1 비교
3. 불일치 영역의 사후 우회 (PR γ §10-2 SoT)

Day 7 PR α 의 Deviation E 미발생 영역과 대조 — 실 응답 schema 영역의 사후 우회는 매 PR 마다 검증 의무 (자동으로 미발생 가정 X).

### 7-4. hygiene 영역의 scope discipline 분리 (3 영역 → 2 PR)

본 archive §5 SoT — hygiene 분할 영역 = 자연 경계 영역 분리. PR scope discipline 의 정공:

- main HEAD baseline 회귀 (별도 PR 분리)
- 의존성 영역 (hotfix #46 의 default 변경 → runbook 갱신)
- 자동화 영역 정착 (web/.eslintrc.json)

Day 7 PR β archive §10-5 의 "자동화 영역 vs 사용자 수동 영역 분리" SoT 의 backend / frontend 확장형.

### 7-5. Plan 의 implementer dispatch + reviewer 분리 패턴 정공 (Day 8 5 PR 영역)

본 Day 8 영역의 5 PR 모두 다음 패턴 정공 reuse:
...
Implementer + reviewer 분리 의무 영역 = LOC 영역에 따라 결정:
- 본격 코드 영역 (>200 LOC) = implementer + reviewer 분리
- hygiene / hotfix 영역 (<50 LOC) = 메인 자율 + 직접 검증

Day 7 PR γ §10-1 의 통합 검증 SoT + execution-policy 메모리의 "200 LOC 기준 분할" 정공 reuse.

## 8. 관련 문서

- 후속 archive (Cloudflare 배포 영역 + 스크린샷): [`2026-05-12-day-8-cloudflare-deploy.md`](2026-05-12-day-8-cloudflare-deploy.md)
- Day 7 archive 2건:
...
본 archive §6 의 SLO 영역은 2026-05-12 19:45 at job 2 fire 후 별도 commit 으로 보강 예정. 보강 시점에 본 §9 영역도 갱신.
