...
**서비스 의도** (CLAUDE.md §1 SoT): 서울 활동 시 관심 지역의 실시간 혼잡도와 영업 중인 카페·술집(특히 마감까지 1시간 이상 남은 곳)을 확인·추천하는 작은 실서비스 + 그 서비스의 데이터 플랫폼. cafefinder 류 한국 유사 서비스의 attribution 없는 ToS 위반 패턴 대신 정공 path 로 차별화.

**시스템**: 서울시 공공 실시간 데이터(도시데이터·지하철 도착정보) + Postgres CDC + 익명 사용자 행동 로그(P1B) + 외부 가게 정보(카카오/네이버 영업시간·별점, P1B Day 16) 를 **Kafka 메시지 버스**로 통합하고 **PyFlink streaming + Spark batch + Iceberg(Lakekeeper) + dbt + Airflow + GitHub Actions** 로 처리한다. 운영 비용은 월 $0~$2.
...
- 사용자가 본 프로젝트로 명시한 3가지 목표:
  ...
  3. **비용을 최대한 적게 쓴다.**
- **사용자 발화 5가지 의도** (2026-05-07/05-14 정정, `[[project-identity-correction]]` + `[[deferred-items-post-day10]]` SoT):
  1. **서울 활동 시 관심 지역의 실시간 혼잡도 확인** — 한산한 동네 / 붐비는 동네를 색상으로 즉시 식별 (P1A 핵심).
...
### 6-1. 3단계 기능 진화 (§1 발화 의도 #4)

| 단계 | 시점 | 사용자가 보는 것 |
|---|---|---|
| 1단계 (단순 필터) | P1A Day 8 데모 | "지금 한가하고 마감 1h+ 남은 카페·술집" 단일 화면 — 익명, 회원가입 X |
| 2단계 (개인화) | P1B Day 11-18 | 익명 행동 토픽 + 회원가입 (Day 15) + 북마크 (Day 12) + Web Push 알림 (Day 13) + 외부 가게 정보(카카오/네이버, Day 16) |
| 3단계 (추천 모델) | P2 W3-W6 | 지역/가게 추천 점수 모델 — 혼잡도 + 영업 + 별점 + 사용자 행동 통합 |
...
| 2 | **"지금 한가하고 영업 중인 카페·술집"** — 혼잡도 + 영업시간 (마감까지 1h+) 결합 데모 | **P1A** |
